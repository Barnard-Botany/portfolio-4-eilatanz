...
#Now let's prune phy so that its taxa exactly match the taxa in community - can plot phy before and after to see how it changed too
...
#plot is six imes recording two,  39:00
```{r}
...
#only if comm label is 1 and not 0, just out a dot on the tip, pch = yellow cex = 2 is dot? or square)
...
```{r}
#metric like shannon and j, but since these are all equally diverse we can see phylogenetic distance
#if even and random = more diversity than clumped in same clade, say. But to do this you must have same species richness (SR) 
...
#trees must have lengths if they are to be used in picante
```{r}
...
#(five is highest diversity, 1 and 2 are the same)
...
#like a jacard index but corrected for phylogeny 53:00 in audio
#not independant, need to take into account how species is in the communitu
...
#does an innovation come up more than once? OR just once in one clade? etc tehse are traits AND phylogenetic relationships also 
```
...
#can take a species list from recipes, also, to try to make sure there is one thing from each or use references below - we'll do an outdoor lab later for this do a vegan analysis but go on step further and do a phylogenetic analysis
